...
use \Psr\Http\Message\ResponseInterface as Response;
	...
	$app->get('/getName/{name}/{caldate}/{addquan}/{prodes}/', function (Request $request, Response
	$response, array $args) {
	...
	$data=json_decode($request->getBody());
		...
		$sql = "INSERT INTO adminstock (name, caldate, addquan, prodes)
		VALUES ('". $name ."','". $caldate ."','". $addquan ."','". $prodes ."')";
	...
	if ($result->num_rows > 0) {
		$data=array();
			...
			array_push($data,array("pcode"=>$row["pcode"] ,"name"=>$row["name"] ,"caldate"=>$row["caldate"],"addquan"=>$row["addquan"], "prodes"=>$row["prodes"]));
			}
    ...
    try {
		...
		// set the PDO error mode to exception
			...
			$sql = "UPDATE adminstock set name='". $name ."', caldate='". $caldate ."', addquan='". $addquan ."', prodes='". $prodes ."' where pcode='". $pcode ."'";
